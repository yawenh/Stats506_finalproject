...
During the analysis, the construction time of the buildings are divided into two groups (before and after 1980). I did point estimates and built confidential intervals using replicate weights on the proportion of equipment usage with different construction time. Also the bias are reduced by grouping the commercial buildings according to the climate regions they located in. Finally, I did analysis by visualizing the result table and figures.
   
### II. Data & Methods:   
#### Data:  
The data I used is `2012 US Commercial Building Energy Consumption Survey` ([CBECS](https://www.eia.gov/consumption/commercial/data/2012/index.php?view=microdata)) data set.   
    
#### Methods:   
This project is written using R. `tidyverse` package is used to perform data 
manipulation part, and `ggplot2` package is used to perform the data visualization 
part.    
...
- step 4 - Visualization: By referring to the codebook, I decode the variables and construct table and plots using `knitr` and `ggplot2` packages.  
...
cool_rw %>%
  ...
  ggplot( aes(x = cool_type, y = prop, color = year) ) +
...
heat_rw %>%
  ...
  ggplot( aes(x = heat_type, y = prop, color = year) ) +
  geom_point( position = position_dodge(.2), size = 0.9 ) +
  geom_errorbar(
    aes(ymin = lwr, ymax = upr),
    position = position_dodge(.2), width = 0.3
...
* Heating:  
  ...
  - The use of inside boilers in cold region and mixed-humid region. In marine region, the use of heating pumps increased a lot, while furnaces usage decreases significantly.  
